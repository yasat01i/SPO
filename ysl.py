...
class UserThread:
    def main(self, domain, sandi):
        ...
        ttl = f"{random.randint(1990, 1991)}-{random.randint(10, 12)}-{random.randint(10, 31)}"
        signup = Spotify().create(ua, nama, email, sandi, ttl, gender)
        # print(signup.text)
        if "login_token" in signup.text:
            print(f" Created : {email} | {sandi}\n")
    ...
    def createThread(self):
        try:
            domain = "@cila.my.id"  # set domain here
            sandi = "Bos12345"  # set password here
    ...
    def user_main(self):
        ...
        print(f"{green}\n\n  ()                       \n  /\          _/_   /)     \n /  )  _   __ /  o // __  ,\n/__/__/_)_(_)<__<_//_/ (_/_ {white}  YASCIL{green}\n     /           />     /  \n    '           </     '   \n{white}\n"); UserThread().createThread()
